...
def get_data():
    ...
    os.system(
        "wget https://raw.githubusercontent.com/aangelopoulos/ltt/main/experiments/meps/data/meps_19_reg.csv -O "
        + str(ABSPATH.parent)
        + "/data/meps/raw/meps_19_reg.csv"
    )
...
def optimize_params_GBR(X_train, X_val, y_train, y_val, alpha=0.1):
    ...
    try:
        ...
        optim_df = pd.DataFrame(
            columns=["lr", "n_estimators", "subsample", "max_depth", "cvg"]
        )
        for lr, n_estimators, subsample, max_depth in tqdm(
            itertools.product(lrs, n_ests, subsamples, max_depths)
        ):
    ...
    mean = GradientBoostingRegressor(
        random_state=0,
        learning_rate=optim_df_mean["lr"],
        n_estimators=int(optim_df_mean["n_estimators"]),
        subsample=optim_df_mean["subsample"],
        max_depth=int(optim_df_mean["max_depth"]),
        alpha=1 - alpha / 2,
        loss="quantile",
    )
    upper = GradientBoostingRegressor(
        random_state=0,
        learning_rate=optim_df_quantiles["lr"],
        n_estimators=int(optim_df_quantiles["n_estimators"]),
        subsample=optim_df_quantiles["subsample"],
        max_depth=int(optim_df_quantiles["max_depth"]),
        alpha=1 - alpha / 2,
        loss="quantile",
    )
    lower = GradientBoostingRegressor(
...
def save_model_outputs():
    ...
    np.savez(
        str(ABSPATH.parent) + "/data/meps/meps-gbr.npz",
        pred=pred_val,
        upper=upper_val,
        lower=lower_val,
        X=X_val,
