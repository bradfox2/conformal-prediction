...
def load_models(utils_dir):
    if not os.path.exists(utils_dir + "/baseline-models/seed10.cbm"):
        ...
        os.system(
            "wget https://storage.yandexcloud.net/yandex-research/shifts/weather/baseline-models.tar -O "
            + utils_dir
            + "/raw_models.tar"
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(
        data_dir + "/canonical-partitioned-dataset/shifts_canonical_dev_in.csv"
    ):
        os.makedirs(data_dir, exist_ok=True)
        os.system(
            "wget https://storage.yandexcloud.net/yandex-research/shifts/weather/canonical-partitioned-dataset.tar -O "
            + data_dir
            + "raw.tar"
        )
        os.system("tar -xf " + data_dir + "/raw.tar -C " + data_dir)
    df_dev_in = pd.read_csv(
        data_dir + "/canonical-partitioned-dataset/shifts_canonical_dev_in.csv"
    )
    df_dev_out = pd.read_csv(
        data_dir + "/canonical-partitioned-dataset/shifts_canonical_dev_out.csv"
    )
    ...
    np.savez(
        save_dir + "/weather-catboost.npz",
        preds=all_preds,
        temperatures=df_dev["fact_temperature"],
        times=df_dev["fact_time"],
    )
