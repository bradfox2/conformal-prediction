...
def get_sigmoids_targets(model, folders):
    ...
    for data_path in folders:
        ...
        for i in tqdm(range(test_loader.size)):
            ...
            gt_masks[k, :, :] = (
                resize(gt, (test_size, test_size), anti_aliasing=False) > 0.5
            )
            k += 1
...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        folders = [
            "/home/aa/Code/conformal-risk-control/polyps/PraNet/data/TestDataset/" + x
            for x in folders
        ]
        # Setup model
        ...
        example_indexes = np.random.choice(
            img_paths.shape[0], size=(500,), replace=False, p=None
        ...
        np.savez(
            str(ABSPATH.parent) + "/data/polyps/polyps-pranet.npz",
            sgmd=sigmoids,
            targets=targets,
        ...
        for idx in example_indexes:
            shutil.copy(
                img_paths[idx],
                str(ABSPATH.parent) + "/data/polyps/examples/" + str(idx) + ".jpg",
            )
            shutil.copy(
                mask_paths[idx],
                str(ABSPATH.parent)
                + "/data/polyps/examples/"
                + str(idx)
                + "_gt_mask.jpg",
            )
