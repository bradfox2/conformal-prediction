...
if __name__ == "__main__":
    with torch.no_grad():
        ...
        imagenet_loader = torch.utils.data.DataLoader(
            imagenet_dataset, batch_size=512, shuffle=False, pin_memory=True
        )
        ...
        example_indexes = np.random.choice(
            len(imagenet_dataset), size=(500,), replace=False, p=None
        ...
        for batch in tqdm(imagenet_loader):
            scores[counter : counter + batch[0].shape[0], :] = (
                model(batch[0].cuda()).softmax(dim=1).cpu().numpy()
            )
        ...
        np.savez(
            str(ABSPATH.parent) + "/data/imagenet/imagenet-resnet152.npz",
            smx=scores,
            labels=labels,
        ...
        for idx in example_indexes:
            shutil.copy(
                paths[idx],
                str(ABSPATH.parent) + "/data/imagenet/examples/" + str(idx) + ".jpeg",
            )
